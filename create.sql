DROP TABlE parking_violation;

CREATE TABLE parking_violation (
    summons_number BIGINT PRIMARY KEY,
    plate_id VARCHAR(10),
    registration_state CHAR(2),
    plate_type VARCHAR(3),
    issue_date DATE,
    violation_code INT,
    vehicle_body_type VARCHAR(4),
    vehicle_make VARCHAR(10),
    issuing_agency CHAR(1),
    street_code1 INT,
    street_code2 INT,
    street_code3 INT,
    vehicle_expiration_date DATE,
    violation_location VARCHAR(10),
    violation_precinct INT,
    issuer_precinct INT,
    issuer_code INT,
    issuer_command VARCHAR(4),
    issuer_squad VARCHAR(4),
    violation_time VARCHAR(5),
    time_first_observed VARCHAR(5),
    violation_county CHAR(2),
    violation_in_front_of_or_opposite CHAR(1),
    house_number VARCHAR(10),
    street_name VARCHAR(50),
    intersecting_street VARCHAR(50),
    date_first_observed DATE,
    law_section VARCHAR(10),
    sub_division VARCHAR(10),
    violation_legal_code VARCHAR(10),
    days_parking_in_effect VARCHAR(50),
    from_hours_in_effect VARCHAR(10),
    to_hours_in_effect VARCHAR(10),
    vehicle_color VARCHAR(10),
    unregistered_vehicle CHAR(1),
    vehicle_year INT,
    meter_number VARCHAR(10),
    feet_from_curb INT,
    violation_post_code VARCHAR(10),
    violation_description VARCHAR(100),
    no_standing_or_stopping_violation CHAR(1),
    hydrant_violation CHAR(1),
    double_parking_violation CHAR(1)
);

\copy parking_violation FROM '/tmp/parking_violation.csv' DELIMITER ',' CSV HEADER;
